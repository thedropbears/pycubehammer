...
class Turret:
    ...
    def __init__(self) -> None:
        ...
        self.motor = CANSparkMax(SparkMaxIds.turret_motor, CANSparkMax.MotorType.kBrushless) #This will need to change to the proper motor controller type
        ...
        self.negative_limit_switch= DigitalInput(DioChannels.negative_turret_switch) # These could also be attached directly to the motor controller breakout board for interrupts
        self.encoder = self.motor.getEncoder()
        self.encoder.setPositionConversionFactor(GEAR_RATIO)
        self.encoder.setVelocityConversionFactor(GEAR_RATIO)
        rotation_contraints = TrapezoidProfileRadians.Constraints(maxVelocity=MAX_ANGULAR_VELOCITY,maxAcceleration=MAX_ANGULAR_ACCELERATION)
        self.rotation_controller = ProfiledPIDControllerRadians(2,0.0,0.0,rotation_contraints)
    ...
    def execute(self) -> None:
        ...
        if self.index_found:
            ...
            pid_output = self.rotation_controller.calculate(self.get_angle(), self.goal_angle)
